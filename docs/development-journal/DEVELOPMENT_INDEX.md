...
| 2026-08-20 | Persistence | Added durable file-backed persistence: a new `packages/persistence` primitive (atomic JSON read/write) plus six `File*Repository` adapters and a `FileAuthProvider`, added alongside every existing `InMemory*` implementation with no domain service/route/contract changes. Verified via automated restart-simulation tests and a real process kill-and-restart against a live server | [2026-08-20-development-log.md](daily/2026-08-20-development-log.md) (3rd continued section) |
| 2026-08-22 | Local release readiness | Made the app easy to run locally with durable data: full `.env.example` for both `apps/api`/`apps/web`, automatic `.env` loading via Node's native `--env-file-if-exists` (no `dotenv` dependency), a safety-checked `reset-data` script, a new `RUNNING.md` guide with a first-run walkthrough and verification checklist, and corrected two stale app-level READMEs. Explicitly local/dev only. Verified live: signup → goal → hard restart → data intact → reset-data → clean slate | [2026-08-22-development-log.md](daily/2026-08-22-development-log.md) |
| 2026-08-22 | Cloud dev/preview planning | Documented (not deployed) a Vercel (web) + Render (API) preview environment: `docs/environments.md` with exact Render/Vercel settings, env vars per side, and a deployment-readiness checklist. Verified two real gotchas against current Render docs rather than memory - free-tier services can't attach a persistent disk at all (Starter plan, $7/mo, required), and Render's injected `PORT` var doesn't match `apps/api`'s `API_PORT` unless both are set explicitly. Persistence tradeoff (pay for a disk vs. accept resets) left as an open decision, not resolved | [2026-08-22-development-log.md](daily/2026-08-22-development-log.md) (continued section) |
| 2026-08-22 | Vercel/Render deployment readiness | Fixed `apps/api`'s port resolution (`PORT` now wins over `API_PORT`, matching how Render actually injects a port) so the earlier "set both manually" workaround is no longer needed - verified live across four port-precedence scenarios against a real running server. Tightened `docs/environments.md` to match, added an explicit "MVP/dev-preview, not production" statement inside the Cloud dev/preview section itself, and reconfirmed the persistence choice is documented but still not decided | [2026-08-22-development-log.md](daily/2026-08-22-development-log.md) (3rd continued section) |
| 2026-08-23 | Roadmap / AI adapter boundary | Built the Roadmap domain (`Roadmap`/`Milestone`/`ActionStep`, validation, persistence, lifecycle) and the `RoadmapGenerator` adapter interface the external AI project (built and validated separately) will plug into later, backed for now by a deterministic `PlaceholderRoadmapGenerator` - no AI provider or API key. Wired into onboarding (first goal auto-generates a roadmap) and Dashboard (`continue_roadmap` next-action tier, per-goal roadmap panel). Verified with 285 passing tests plus a real end-to-end Playwright browser run against live dev servers | [2026-08-23-development-log.md](daily/2026-08-23-development-log.md) |
...
| 2026-08-20 | File-backed durable persistence: one `File*Repository` per existing repository interface plus a `FileAuthProvider` for the separate credential boundary, added alongside (not replacing) every `InMemory*` implementation; `createDefaultDependencies(dataDir?)` - no domain service, route, or contract changed; sessions deliberately not persisted | Accepted | [ADR 0016](../architecture-decisions/0016-file-backed-durable-persistence.md) |
| 2026-08-22 | Local release readiness: `.env` loading via Node's native `--env-file-if-exists` (no `dotenv` dependency), two separate `.env.example` files (root for `apps/api`, `Apps/web/.env.example` for Vite), a safety-checked `reset-data` script over prose instructions, `RUNNING.md` as the single local-setup source of truth; explicitly local/dev only, no production deployment | Accepted | [ADR 0017](../architecture-decisions/0017-local-release-readiness.md) |
| 2026-08-22 | Cloud dev/preview planning: hosting split accepted as already decided (Vercel web / Render API); `docs/environments.md` as the single source for all three environments; Render Root Directory = repo root (avoids a real dependency-hoisting mismatch); persistence tradeoff (paid Render disk vs. accepted resets) presented but deliberately left for the user to decide; no deployment config files or code changes made | Accepted | [ADR 0018](../architecture-decisions/0018-cloud-dev-preview-planning.md) |
| 2026-08-22 | Vercel/Render deployment readiness: `apps/api` now binds to Render's injected `PORT` automatically (`API_PORT` stays the local-dev fallback), removing the manual dual-env-var workaround ADR 0018 had documented; verified live across four port-precedence scenarios; `docs/environments.md` tightened to match; persistence choice documented more clearly but still left for the user to decide | Accepted | [ADR 0019](../architecture-decisions/0019-vercel-render-deployment-readiness.md) |
| 2026-08-23 | Roadmap domain and AI adapter boundary: `RoadmapGenerator` interface as the only seam AI content enters through, enforced "AI proposes, Better You validates and persists" via `validateRoadmapDraft()`, one roadmap per goal with no regeneration/versioning yet, `completeActionStep()` as the only status-changing action, Dashboard gains a real Roadmap dependency (`continue_roadmap` next-action tier) | Accepted | [ADR 0020](../architecture-decisions/0020-roadmap-domain-and-ai-adapter.md) |
